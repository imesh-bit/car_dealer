...
// Best-effort cleanup of a listing's uploaded image files. Any failure here
// is logged but never blocks the actual deletion of the listing record.
const deleteListingImages = async (listing) => {
  ...
  if (Array.isArray(listing?.gallery)) {
    listing.gallery.forEach((g) => {
      if (g?.imageSrc) urls.add(g.imageSrc);
    });
  }
