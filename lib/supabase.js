...
export const normalizeListingRecord = (row = {}) => {
  ...
  const normalizedRow = {
    ...
    packagingType: row.packagingType || row.packaging_type || "",
    orderScale: row.orderScale || row.order_scale || "",
    minimumOrderQuantity: row.minimumOrderQuantity || row.minimum_order_quantity || "",
    partCategory: row.partCategory || row.part_category || "",
    brand: row.brand || "",
